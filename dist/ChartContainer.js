...
const _html2canvas = _interopRequireDefault(require("html2canvas"))

const _ChartNode = _interopRequireDefault(require("./ChartNode"))
...
const ChartContainer = (0, _react.forwardRef)(function (_ref, ref) {
  ...
  const _useState9 = (0, _react.useState)("default"),
  ...
  const _useState13 = (0, _react.useState)(""),
  ...
  const attachRel = function attachRel(data, flags) {
  ...
  const _useState17 = (0, _react.useState)(datasource),
  ...
  const clickChartHandler = function clickChartHandler(event) {
  ...
  const panEndHandler = function panEndHandler() {
    setPanning(false)
    setCursor("default")
  }

  const panHandler = function panHandler(e) {
    let newX = 0
    let newY = 0
  ...
  const panStartHandler = function panStartHandler(e) {
  ...
  const updateChartScale = function updateChartScale(newScale) {
    let matrix = []
    let targetScale = 1
  ...
  const zoomHandler = function zoomHandler(e) {
    const newScale = 1 + (e.deltaY > 0 ? -0.2 : 0.2)
    updateChartScale(newScale)
  }

  const exportPNG = function exportPNG(canvas, exportFilename) {
